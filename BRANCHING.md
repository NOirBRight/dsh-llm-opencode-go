...
|---|---|
| 稳定分支 | `master` |
| 当前稳定版（用户安装 / 回滚首选） | `v0.1.24-015rc1c` |
| 上一稳定版（兜底回滚） | `v0.1.23` |
| 集成分支 | `dev` |
| dev HEAD | `a353182 test: assert real list rows and post-release order in the drag reorder case` |

### 已折叠进 dev 的历史线

- feat/provider-settings-a (older parallel line; keep the v0.1.24 implementation)
- agent/ui-10-register (older parallel line; keep the v0.1.24 implementation)
- feat/provider-reorder (older parallel line; keep the v0.1.24 implementation)
