...
function apply(ctx, config) {
	...
	if (Object.hasOwn(config, "remoteManagement")) throw new Error("llm-opencode-go: remoteManagement is unsupported by the Alpha.4 Host RPC; remove it from the plugin config");
	let current = () => config;
	let lastRaw;
	let lastGood;
	const options = () => {
	...
	ctx.effect(() => {
		return ctx.inject(["connection"], (connectionCtx) => {
			connectionCtx.effect(() => connectionCtx.connection.rpc.handle(OPENCODE_GO_RPC_CHANNEL, async (endpoint, payload, signal) => {
...
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_STREAM_IDLE_TIMEOUT_MS, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, OPENCODE_GO_CREDENTIAL_SET_ENDPOINT, OPENCODE_GO_CREDENTIAL_STATUS_ENDPOINT, OPENCODE_GO_DISCOVER_ENDPOINT, OPENCODE_GO_PROVIDER, OPENCODE_GO_PUBLIC_BASE_URL, OPENCODE_GO_RPC_CHANNEL, OPENCODE_GO_SAVE_ENDPOINT, OPENCODE_GO_SETTINGS_NAMESPACE, OPENCODE_GO_SETTINGS_READ_ENDPOINT, OPENCODE_GO_USAGE_ENDPOINT, OPENCODE_GO_USAGE_FAILED, OPENCODE_GO_USAGE_UNSUPPORTED, OpenCodeGoAdapter, PUBLIC_BASE_URL, apply, chatBaseURLForApi, createOpenCodeGoPiAiProfile, decodeOpenCodeGoCatalogModel, decodeOpenCodeGoCredentialSetRequest, decodeOpenCodeGoDiscoveryRequest, decodeOpenCodeGoDiscoveryResult, decodeOpenCodeGoSaveRequest, decodeOpenCodeGoSaveResult, decodeOpenCodeGoSettings, decodeOpenCodeGoSettingsReadResult, decodeOpenCodeGoUsageReply, discoverModels, enrichModel, familyForModel, inject, knownModel, name, parseOpenCodeGoModels, parseOpenCodeGoUsage, protocolForModel, readOpenCodeGoUsage, resolveAdapterOptions };
