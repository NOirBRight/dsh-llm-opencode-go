...
function decodeOpenCodeGoSaveRequest(value) {
	...
	if (value.apiKey !== void 0 && (typeof value.apiKey !== "string" || value.apiKey.length === 0)) return void 0;
	return {
		baseURL: value.baseURL,
		models,
		expectedRevision,
		...value.apiKey === void 0 ? {} : { apiKey: value.apiKey }
	};
}
				...
				try {
					...
					if (request.apiKey !== void 0) {
						...
						try {
							...
							if (message.includes("writer lock")) return settingsFailure("Could not store the API key because ~/.dsh/.credentials.yaml.lock is held. Delete that stale lock file and retry.");
...
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_STREAM_IDLE_TIMEOUT_MS, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, OPENCODE_GO_DISCOVER_ENDPOINT, OPENCODE_GO_PROVIDER, OPENCODE_GO_PUBLIC_BASE_URL, OPENCODE_GO_RPC_CHANNEL, OPENCODE_GO_SAVE_ENDPOINT, OPENCODE_GO_SETTINGS_NAMESPACE, OPENCODE_GO_USAGE_ENDPOINT, OPENCODE_GO_USAGE_FAILED, OPENCODE_GO_USAGE_UNSUPPORTED, OpenCodeGoAdapter, PUBLIC_BASE_URL, apply, createOpenCodeGoPiAiProfile, decodeOpenCodeGoCatalogModel, decodeOpenCodeGoDiscoveryRequest, decodeOpenCodeGoDiscoveryResult, decodeOpenCodeGoSaveRequest, decodeOpenCodeGoSaveResult, decodeOpenCodeGoSettings, decodeOpenCodeGoUsageReply, discoverModels, enrichModel, familyForModel, inject, knownModel, name, parseOpenCodeGoModels, parseOpenCodeGoUsage, protocolForModel, readOpenCodeGoUsage, resolveAdapterOptions };
