...
*/
function openCodeGoIdentityHeaders() {
	return {
...
function modelUnavailableMessage(message) {
...
function classifyOpenCodeGoTransientError(chunk) {
	...
	if ((failure.code === "AUTH" || failure.code === "PI_AI_ERROR") && modelUnavailableMessage(message)) return {
		...chunk,
		reason: {
			...chunk.reason,
			failure: {
...
function apply(ctx, config) {
	...
	const apiKeyFromRecord = (record) => {
		...
		if (record.kind !== "api-key") throw new LlmError(`llm-opencode-go: credential record "${OPENCODE_GO_CREDENTIAL_KEY}" is not an API-key record`, "INVALID_CREDENTIAL");
	...
	const resolveApiKey = async () => {
		...
		throw new LlmError(`llm-opencode-go: no API key stored in credential record "${OPENCODE_GO_CREDENTIAL_KEY}"; use the OpenCode Go settings card to store it`, "MISSING_CREDENTIAL");
	};
	...
	const credentialStatus = async () => {
		...
		return {
			configured: await storedApiKey() !== void 0,
		...
		connectionCtx.connection.rpc.handle(OPENCODE_GO_RPC_CHANNEL, async (endpoint, payload, signal) => {
			...
			if (endpoint === "credentials/set") {
				...
				await credentials.modifyRecord(OPENCODE_GO_CREDENTIAL_KEY, async (current) => {
					if (current !== void 0 && current.kind !== "api-key") throw new LlmError(`llm-opencode-go: credential record "${OPENCODE_GO_CREDENTIAL_KEY}" is not an API-key record`, "INVALID_CREDENTIAL");
					return {
						kind: "api-key",
						key: request.apiKey
					};
