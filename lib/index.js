...
function apply(ctx, config) {
	if (Object.hasOwn(config, "remoteManagement")) throw new Error("llm-opencode-go: remoteManagement is unsupported by the alpha.1 Host RPC; remove it from the plugin config");
	let current = () => config;
	let lastRaw;
	let lastGood;
	const options = () => {
