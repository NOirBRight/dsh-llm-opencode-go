...
/** Credential reference used by releases before the official OpenCode name. */
const LEGACY_API_KEY_ENV = "OPENCODE_GO_API_KEY";
...
function apply(ctx, config) {
	...
	options();
	const compatibleCredentialRefs = (ref) => ref === DEFAULT_API_KEY_REF ? [ref, LEGACY_API_KEY_REF] : [ref];
	const resolveApiKey = async (connection) => {
		const ref = connection.apiKeyEnv;
		const credentials = ctx.get("credentials");
		if (credentials !== void 0) for (const candidate of compatibleCredentialRefs(ref)) {
			const hit = await credentials.resolve(candidate);
			if (hit !== void 0) return assertUsableApiKey(hit.value, "llm-opencode-go", candidate);
		}
		else for (const candidate of compatibleCredentialRefs(ref)) {
			const ambient = launchEnvironmentOf(ctx).get(candidate);
			if (ambient !== void 0 && ambient.value.length > 0) return assertUsableApiKey(ambient.value, "llm-opencode-go", candidate);
		}
	...
	const credentialStatus = async () => {
		...
		if (info.configured || ref !== DEFAULT_API_KEY_REF) return {
			configured: info.configured,
			writable: info.writable
		};
		return {
			configured: (await credentials.describe(LEGACY_API_KEY_REF)).configured,
			writable: info.writable
		};
	};
