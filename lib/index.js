...
async function loadOpenCodeGoModelsDev(fetchImpl = fetch, signal) {
	hydrateFromDisk();
	if (cache !== void 0 && Date.now() - cache.at < REFRESH_AFTER_MS) return cache.overlay;
	if (inflight !== void 0) return inflight;
	inflight = fetchAndStore(fetchImpl, signal).finally(() => {
		inflight = void 0;
	});
	if (cache !== void 0) return cache.overlay;
...
var OpenCodeGoAdapter = class extends LlmAdapter {
	...
	*/
	...
	async *stream(options) {
		for await (const chunk of this.current().stream(narrowOpenCodeGoEscalationSchemas(options))) yield classifyOpenCodeGoTransientError(chunk);
	}
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: async function* (options) {
				for await (const chunk of inner.stream(narrowOpenCodeGoEscalationSchemas(options))) yield classifyOpenCodeGoTransientError(chunk);
			}
		};
	}
};
