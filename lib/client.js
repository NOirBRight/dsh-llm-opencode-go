		...
		*/
		...
		function apply(ctx) {
			...
			const saveConfiguration = async (settings, apiKey) => {
				...
				if (current.status !== "ready" || current.value === void 0 || current.revision === void 0 || !current.writable) throw new Error(t("requestFailed"));
				const ops = [];
				...
				if (ops.length > 0 && !await openCodeGoSettings.mutate(ops, current.revision)) throw new Error(t("requestFailed"));
