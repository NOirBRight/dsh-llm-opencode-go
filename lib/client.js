		...
		*/
		...
		function apply(ctx) {
			...
			const saveConfiguration = async (settings, apiKey) => {
				...
				const saved = await rpc.call(OPENCODE_GO_RPC_CHANNEL, OPENCODE_GO_SAVE_ENDPOINT, {
					baseURL: settings.baseURL,
			...
			const fetchUsage = async (request) => {
				const result = await rpc.call(OPENCODE_GO_RPC_CHANNEL, OPENCODE_GO_USAGE_ENDPOINT, request);
				if (!result.ok) {
			...
			const discoverModels = async (request) => {
				const result = await rpc.call(OPENCODE_GO_RPC_CHANNEL, OPENCODE_GO_DISCOVER_ENDPOINT, request);
