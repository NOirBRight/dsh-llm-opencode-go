		...
		function OpenCodeGoPluginCard(props) {
			...
			const patchModel = (index, patch) => {
				...
				patchDraft({ models: draft.models.map((model, at) => {
					...
					if (patch.id !== void 0) {
						next.id = patch.id;
						if (next.thinking === true && next.defaultEffort !== void 0) {
							const supported = openCodeGoSupportedEfforts({
								id: next.id,
								thinking: true
							});
							if (!supported.includes(next.defaultEffort)) {
					...
					if ("thinking" in patch) {
						...
						else if (next.defaultEffort === void 0) {
							const fallback = openCodeGoDefaultEffort(next.id) ?? openCodeGoSupportedEfforts({
								id: next.id,
								thinking: true
							})[0];
							if (fallback !== void 0) next.defaultEffort = fallback;
														...
														children: [
															...
															model.thinking === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
																...
																children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
																	...
																	style: selectStyle,
																	value: model.defaultEffort ?? openCodeGoDefaultEffort(model.id) ?? openCodeGoSupportedEfforts({
																		id: model.id,
																		thinking: true
																	})[0] ?? "",
																	disabled,
