	...
	factory: (require) => {
		...
		*/
		...
		function apply(ctx) {
			...
			const withTimeout = async (work, ms) => {
				let timer;
				try {
			...
			const storeApiKey = async (value) => {
				const ref = scope.getSnapshot().value?.apiKeyEnv ?? "OPENCODE_GO_API_KEY";
				const response = await withTimeout(connectionApi.credentials.set({
					ref,
					value
				}, AbortSignal.timeout(15e3)), 15e3);
				if (!response.result.ok) throw new Error(response.result.error.message);
			};
			const saveConfiguration = async (settings, apiKey) => {
				...
				const ops = [];
				if (snapshot.value.baseURL !== settings.baseURL) ops.push({
					op: "set",
					path: ["baseURL"],
					value: settings.baseURL
				});
				if (JSON.stringify(snapshot.value.models) !== JSON.stringify(settings.models)) ops.push({
					op: "set",
					path: ["models"],
				...
				const response = await withTimeout(connectionApi.settings.mutate({
				...
				if (decoded === void 0) throw new Error(t("requestFailed"));
				return {
					settings: decoded,
