...
describe('OpenCode Go pi-ai profile', () => {
  it('dispatches Completions, Responses, and Messages from one provider', () => {
    ...
    expect(models.find(model => model.id === 'glm-5.3')?.baseUrl).toBe('https://opencode.ai/zen/go/v1')
    expect(models.find(model => model.id === 'grok-4.6')?.baseUrl).toBe('https://opencode.ai/zen/go/v1')
    expect(models.find(model => model.id === 'minimax-m3')?.baseUrl).toBe('https://opencode.ai/zen/go')
